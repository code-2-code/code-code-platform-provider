...
func (p *oauthFinalizePlan) ValidateExisting(existing *ProviderView) error {
	surface := p.CreateProvider()
	if existing == nil {
		return domainerror.NewValidation("platformk8s/providerconnect: existing provider is nil")
	}
	if surface == nil {
		return domainerror.NewValidation("platformk8s/providerconnect: provider is nil")
	}
...
func validateSurfaceFieldMatch(surfaceID, field, current, next string) error {
	...
	return domainerror.NewAlreadyExists(
		"platformk8s/providerconnect: provider surface %q already exists with different %s",
		surfaceID,
		field,
	)
}
