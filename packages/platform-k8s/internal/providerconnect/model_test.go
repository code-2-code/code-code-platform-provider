...
func TestNewConnectPlanAssignsSharedIdentityToTargets(t *testing.T) {
	...
	targetB := newConnectTargetWithIDs(
		...
		"",
		"",
		testProviderView("anthropic", "Anthropic"),
	)
...
func TestNewSessionRecordProjectsOAuthStatusAndTargetRoundTrip(t *testing.T) {
	...
	instance := record.target(decoded).Provider("credential-imported")
...
func testProviderView(_ string, displayName string) *providerv1.ProviderSurfaceRuntime {
