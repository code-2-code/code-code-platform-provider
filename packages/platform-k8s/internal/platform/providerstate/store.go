...
func (r *ProviderRepository) List(ctx context.Context) ([]*providerv1.Provider, error) {
	if r == nil || r.resources == nil {
		return nil, fmt.Errorf("platformk8s/providerstate: repository is not initialized")
	}
...
func (r *ProviderRepository) Get(ctx context.Context, providerID string) (*providerv1.Provider, error) {
	...
	if providerID == "" {
		return nil, domainerror.NewValidation("platformk8s/providerstate: provider id is empty")
	}
...
func (r *ProviderRepository) Update(ctx context.Context, providerID string, mutate func(*providerv1.Provider) error) (*providerv1.Provider, error) {
	...
	if mutate == nil {
		return nil, fmt.Errorf("platformk8s/providerstate: mutate function is nil")
	}
...
func (r *ProviderRepository) Delete(ctx context.Context, providerID string) error {
	if r == nil || r.resources == nil {
		return fmt.Errorf("platformk8s/providerstate: repository is not initialized")
	}
	providerID = strings.TrimSpace(providerID)
	if providerID == "" {
		return domainerror.NewValidation("platformk8s/providerstate: provider id is empty")
	}
...
func marshalNormalizedProvider(provider *providerv1.Provider) ([]byte, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("platformk8s/providerstate: marshal provider %q: %w", id, err)
	}
...
func unmarshalProvider(id string, payload []byte) (*providerv1.Provider, error) {
	...
	if err := (protojson.UnmarshalOptions{DiscardUnknown: true}).Unmarshal(payload, provider); err != nil {
		return nil, fmt.Errorf("platformk8s/providerstate: unmarshal provider %q: %w", id, err)
	}
	return NormalizeProviderForRead(id, provider)
}

// NormalizeProviderForWrite validates and normalizes a provider for persistence.
func NormalizeProviderForWrite(id string, input *providerv1.Provider) (*providerv1.Provider, error) {
	...
	if err := providerv1.ValidateProvider(provider); err != nil {
		return nil, domainerror.NewValidation("platformk8s/providerstate: invalid provider %q: %v", provider.GetProviderId(), err)
	}
	return provider, nil
}

// NormalizeProviderForRead normalizes a provider for read operations.
func NormalizeProviderForRead(id string, input *providerv1.Provider) (*providerv1.Provider, error) {
	if input == nil {
		return nil, domainerror.NewValidation("platformk8s/providerstate: provider is nil")
	}
	...
	if id != "" && provider.GetProviderId() != id {
		return nil, domainerror.NewValidation("platformk8s/providerstate: provider id %q does not match stored id %q", provider.GetProviderId(), id)
	}
	return provider, nil
}

// ProviderNotFound returns a domain not-found error for the given provider.
func ProviderNotFound(providerID string) error {
	return domainerror.NewNotFound("platformk8s/providerstate: provider %q not found", providerID)
}

// CloneProviderProto deep-copies a provider proto message.
