// Package grpcerrors provides a shared domain-error to gRPC-status mapping
// used by all platform gRPC service handlers.
package grpcerrors
...
// MapError translates a Go domain error into the corresponding gRPC status error.
// If err is already a gRPC status error it is returned unchanged.
